                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                    {(data?.items ?? []).map((testimonial, index) => (
                        <AnimatedSection key={index} className="flex flex-col">
                            ...
                            <h4 className="text-[32px] font-serif italic text-vlanc-black mb-4">{testimonial.name}</h4>
                            {/* Cuerpo Negro */}
                            <p className="text-vlanc-black text-[12px] leading-relaxed text-justify">"{testimonial.quote}"</p>
                        </AnimatedSection>
                    ))}
                </div>
            </div>
